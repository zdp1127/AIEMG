...
class pareto:
    ...
    def Update(self,scores,compound):
        ...
        if self.cliff_detector is not None:
            ...
            # Calculate ACS score (after cliff detection)
            if len(scores) >= 5:  # Ensure ACS score position exists
                scores[4] = _calculate_acs(scores[0], self.cliff_detector, compound, compound_scores=scores)
                print(f"Calculated ACS score: {scores[4]:.3f} (docking: {scores[0]:.3f})")
        else:
            # If no cliff detector, use normalized docking score as ACS
            if len(scores) >= 5:
                scores[4] = _calculate_acs(scores[0], None, compound)
                print(f"Using docking score as ACS: {scores[4]:.3f}")
...
class Node:
    ...
    def hvcal(self,pareto,ucb):
        ...
        ref_point = [0,0,0,0,0]  # Extend to 5 dimensions
        hvnum = 0
        try:
...
def _calculate_acs(docking_score: float, cliff_detector, smiles: str, compound_scores=None) -> float:
    """
    Calculate ACS (Activity Cliff Score)
    Support ACR component ablation via global parameters: DISABLE_ACR_COMPONENT / weights ACS_DOCKING_WEIGHT, ACS_ACR_WEIGHT
    """
    ...
    acr = _calculate_acr(smiles, cliff_detector, compound_scores=compound_scores)
