...
def _createModel(vocab_size: int, embed_size: int, N: int):
    """
        Create NN Model (matches model3 architecture).
        - Sequential API
        - No Dropout
    ...
    model = Sequential([
        Embedding(input_dim=vocab_size, output_dim=embed_size, input_length=N, mask_zero=False),
        GRU(units=256, activation='sigmoid', return_sequences=True, reset_after=False),
        GRU(units=256, activation='sigmoid', return_sequences=True, reset_after=False),
        TimeDistributed(Dense(embed_size, activation='softmax'))
    ])
    return model
...
if __name__ == "__main__":
    ...
    X = pad_sequences(X_train, maxlen=82, dtype='int32',
        padding='post', truncating='pre', value=0.)
    y = pad_sequences(y_train, maxlen=82, dtype='int32',
        padding='post', truncating='pre', value=0.)
